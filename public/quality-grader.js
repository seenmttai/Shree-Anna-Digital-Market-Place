(function(){
  ...
  const DefaultCfg = {
    ...
    // extra implicit thresholds used in classifier
    tinyAreaRatio: 0.25,
    hugeAreaRatio: 2.5,
    foreignSolidityMax: 0.65,
    aspectMax: 4.0,
    colorZForeign: 3.5
  };

  function estimateBackgroundFromBorder(L){
  ...
  function initialMask(L, hsv, bg){
    ...
    const colorAid = new cv.Mat(); cv.bitwise_or(sMask, vMask, colorAid); cv.bitwise_or(mask, colorAid, mask);
    hsvSplit.delete(); S.delete(); V.delete(); sMask.delete(); vMask.delete(); colorAid.delete();
    return mask;
  }
  ...
  function watershedSplit(binaryMask, srcColor, cfg){
    ...
    const markers = new cv.Mat(); cv.connectedComponents(dtThresh, markers);

    const unknown = new cv.Mat(); cv.subtract(sureBg, dtThresh, unknown);
    // shift labels by +1; zero unknowns
    ...
    srcBGR.delete(); sureBg.delete(); dist.delete(); dist8u.delete(); dtThresh.delete(); unknown.delete();
    return markers;
  }

  function extractComponents(markers, labImg, hsvImg, cfg){
    ...
    for (const [label, pixCount] of counts.entries()){
      ...
      for (let i=0; i<h*w; i++){ if (data[i] === label) mdata[i] = 255; }
      ...
      comps.push({
        ...
        Lstd: LstdVal, centerContrast
      });

      hull.delete(); contours.delete(); hierarchy.delete(); one.delete(); mask.delete(); maskPoly.delete();
      labSplit.delete(); Lmean.delete(); Lstd.delete(); strip.delete(); centerMask.delete();
    }
    return comps;
  }
  ...
  function summarizeAndGrade(items, cfg){
    ...
    return { grade, counts, total };
  }

  function analyzeRGBA(srcRGBA, cfg){
    ...
    let clahe=null; try { clahe = new cv.CLAHE(2.0, new cv.Size(8,8)); } catch{}
    ...
    const bg = estimateBackgroundFromBorder(L);
    const mask0 = initialMask(L, hsv, bg);

    const maskClean = new cv.Mat(); mask0.copyTo(maskClean);
    const kOpen = cv.Mat.ones(3,3, cv.CV_8U), kClose = cv.Mat.ones(3,3, cv.CV_8U);
    ...
    const markers = watershedSplit(maskClean, srcFixed, cfg);
    const comps = extractComponents(markers, labFixed, hsv, cfg);
    const classified = classifyComponents(comps, cfg);
    const { grade } = summarizeAndGrade(classified, cfg);

    // cleanup
    src.delete(); lab.delete(); hsv.delete(); labSplit.delete(); labFixed.delete(); srcFixed.delete();
    L.delete(); A.delete(); B.delete(); mask0.delete(); maskClean.delete(); markers.delete();
  ...
  async function loadImageToCanvas(url) {
    return new Promise((resolve, reject) => {
      ...
      img.onload = () => {
        ...
        cvEl.width = img.naturalWidth;
        cvEl.height = img.naturalHeight;
  ...
  async function gradeFromUrl(url){
    await ensureCVReady();
    const canvas = await loadImageToCanvas(url);
    const srcRGBA = cv.imread(canvas);
    const grade = analyzeRGBA(srcRGBA, DefaultCfg);
    srcRGBA.delete();
    return grade;
  }
  ...
  function ensureCVReady(){
    return new Promise((resolve) => {
      ...
      const hook = () => { cvReady = true; resolve(); };
      if (window.cv && cv['onRuntimeInitialized'] !== undefined) {
        cv['onRuntimeInitialized'] = hook;
      } else if (window.Module) {
        Module['onRuntimeInitialized'] = hook;
      } else {
        // fallback wait loop
        const t = setInterval(() => {
          if (window.cv && cv.Mat) { clearInterval(t); cvReady = true; resolve(); }
        }, 50);
      }
    });
  }

  window.QualityGrader = { gradeFromUrl };
})();